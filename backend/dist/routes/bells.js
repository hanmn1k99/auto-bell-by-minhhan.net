...
router.get('/template', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.post('/import', auth_1.authenticateToken, upload.single('file'), async (req, res) => {
    if (!req.file)
    ...
    try {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            // Find references
            let department = await prisma_1.prisma.department.findUnique({ where: { name: depName } });
            if (!department) {
                department = await prisma_1.prisma.department.create({ data: { name: depName } }); // Auto create if not exist
            }
            const audioFile = await prisma_1.prisma.audioFile.findUnique({ where: { filename: audioFileName } });
            if (!audioFile) {
            ...
            if (isNaN(volume))
                volume = 1.0;
            await prisma_1.prisma.bellConfig.create({
                data: {
...
router.post('/bulk-update', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        await prisma_1.prisma.bellConfig.updateMany({
...
router.post('/bulk-update-audio', auth_1.authenticateToken, async (req, res) => {
    try {
        const { ids, audioFileId, isActive } = req.body;
        if (!ids || !Array.isArray(ids) || ids.length === 0) {
...
router.post('/bulk-delete', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        await prisma_1.prisma.bellConfig.deleteMany({
            where: { id: { in: ids.map(Number) } },
        });
