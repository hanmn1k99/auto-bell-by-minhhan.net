...
export default function PlayerPage() {
  ...
  const [interacted, setInteracted] = useState(
    () => localStorage.getItem("skipStart") === "1",
  );
  ...
  useEffect(() => {
    ...
    const t = setInterval(
      () => setCurrentTime(new Date(Date.now() + timeOffset.current)),
      1000,
    );
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/files/assets/info`)
      .then((r) => r.json())
      .then((data) => {
        if (data.logo) setLogoUrl(`${API_URL}${data.logo}`);
      })
      .catch(() => {});
  }, []);
  ...
  const schedulePlay = (
    ...
    if (localStorage.getItem("isSimulatorMode") === "true") {
      try {
        ...
        if (
          (audioEl as any)._audioCtx &&
          (audioEl as any)._audioCtx.state === "suspended"
        ) {
          (audioEl as any)._audioCtx.resume().catch(() => {});
        }
        if ((audioEl as any)._panner) {
          if (soundCardId === "card-1")
            (audioEl as any)._panner.pan.value = -1; // Kênh Trái
          else if (soundCardId === "card-2")
            (audioEl as any)._panner.pan.value = 1; // Kênh Phải
    ...
    const fadeTime =
      fadeInDuration !== undefined ? fadeInDuration * 1000 : 1000;
  ...
  useEffect(() => {
    const registerDevice = () => {
      ...
      if (!wanIpRef.current) {
        fetch("https://api4.ipify.org?format=text", {
    ...
    socket.on(
      "DEVICE_STATUS",
      (data: { isApproved: boolean; name?: string }) => {
            ...
            setTimeout(() => {
              win.postMessage(
                JSON.stringify({
                  event: "command",
                  func: "setOption",
                  args: ["captions", "track", { languageCode: "vi" }],
                }),
                "*",
              );
            }, 500);
    ...
    const isForThisDevice = (targetSoundCardId?: string) => {
      if (
        !targetSoundCardId ||
        ["default", "all", "card-1", "card-2"].includes(targetSoundCardId)
      )
        return { forMe: true, realCardId: targetSoundCardId };
      const parts = targetSoundCardId.split("::");
      if (parts.length === 2) {
        const [tDeviceId, tCardId] = parts;
        if (tDeviceId !== getDeviceId())
          return { forMe: false, realCardId: undefined };
    ...
    socket.on("PLAY_BELL", (data: AudioEvent) => {
      ...
      if (
        ...
        originalVolumeRef.current =
          nowPlaying?.volume !== undefined ? nowPlaying.volume : 1.0;
    ...
    socket.on(
      "SYNC_STATE",
      (data: {
        currentTrack: { path: string; name: string } | null;
        volume?: number;
        isOverride?: boolean;
        targetTime?: number;
        status?: string;
        ...
        setNowPlaying((prev) => {
          if (
            prev?.targetTime === data.targetTime &&
            prev?.status === data.status &&
            prev?.url === data.currentTrack?.path
          ) {
            return prev;
          }
          return evt;
        ...
        if (
          data.status === "paused" &&
          data.pauseOffset !== undefined &&
          audioRef.current
        ) {
          if (audioTimeout.current) clearTimeout(audioTimeout.current);
          audioRef.current.pause();
          const fullUrl = evt.url.startsWith("http")
            ? evt.url
            : `${API_URL}${evt.url}`;
          ...
          setTimeout(() => {
            if (audioRef.current)
              audioRef.current.currentTime = data.pauseOffset as number;
          }, 50);
        } else {
    ...
    >
      ...
      {blockedUntil && (
        <div className="interaction-overlay">
          ...
          >
            ...
            >
              {React.createElement("ion-icon", { name: "hand-right" })}
            </div>
      ...
      {!blockedUntil && isRejected && (
        <div className="interaction-overlay">
          ...
          >
            ...
            >
              {React.createElement("ion-icon", { name: "ban" })}
            </div>
            ...
            >
              ...
              >
                {React.createElement("ion-icon", { name: "key-outline" })} Đổi
                ID
              </button>
            </div>
          </div>
      ...
      {!blockedUntil && !isRejected && !interacted && (
        <div className="interaction-overlay" style={{ zIndex: 9999 }}>
          <div className="interaction-box">
            <div style={{ fontSize: "3rem", color: "var(--accent)" }}>
              {React.createElement("ion-icon", { name: "finger-print" })}
            </div>
            <h2>Bấm vào màn hình để bắt đầu</h2>
            <p>
              Trình duyệt yêu cầu tương tác để có thể phát âm thanh tự động.
            </p>
      ...
      {!blockedUntil && isApproved === null && (
        <div
          ...
          style={{ zIndex: 9999, background: "rgba(11, 15, 26, 0.95)" }}
        >
          <div className="interaction-box">
            <div
              className="loading"
            ...
            <p style={{ opacity: 0.7 }}>
              Hệ thống đang xác thực thiết bị của bạn với máy chủ trung tâm.
            </p>
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            <div style={{ fontSize: "3rem", color: "#ef4444" }}>
              {React.createElement("ion-icon", { name: "lock-closed" })}
            </div>
            <h2 style={{ color: "#ef4444" }}>Thiết bị chưa được cấp quyền</h2>
            <p>Vui lòng liên hệ Quản trị viên để phê duyệt thiết bị này.</p>
            <div
              style={{
                marginTop: "1.5rem",
                display: "flex",
      ...
      {promptDialog && (
        <div
          className="interaction-overlay"
          style={{ zIndex: 999999, background: "rgba(11, 15, 26, 0.98)" }}
        >
          <div
            className="interaction-box"
            style={{
          ...
          >
            ...
            <p
              style={{ opacity: 0.8, marginBottom: "1.5rem", fontSize: "1rem" }}
            >
              Trường hợp máy bị đóng băng hoặc xóa cookie làm đổi ID liên tục,
              hãy nhập một ID cố định (Ví dụ: <strong>khu-a-tang-1</strong>).
            </p>
            <form
              onSubmit={(e) => {
                e.preventDefault();
      ...
      <div className="player-container">
        <header className="player-header">
          {logoUrl ? (
            ...
            <>
              <div className="player-logo-placeholder">
                {React.createElement("ion-icon", { name: "notifications" })}
              </div>
          ...
          <div
            className={`player-status-dot ${connected ? "connected" : "disconnected"}`}
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    backgroundColor:
                      bellPlaying?.soundCardId === "card-1" ||
                      bellPlaying?.soundCardId === "all" ||
                ...
                <div>
                  ...
                  <div
                    style={{ display: "flex", gap: "3px", marginTop: "3px" }}
                  >
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    backgroundColor:
                      bellPlaying?.soundCardId === "card-2" ||
                      bellPlaying?.soundCardId === "all"
                        ? "#3b82f6"
                        : "#334155",
                    boxShadow:
        ...
        <footer className="player-footer">
          {bellPlaying ? (
            <div className="player-bell-alert">
              ...
              <div>
                <div className="bell-type">
                  {bellPlaying.type || "Tiếng chuông"}
                </div>
            ...
            <div className="player-idle">
              <span style={{ marginRight: "8px" }}>
                {React.createElement("ion-icon", { name: "hourglass-outline" })}
              </span>{" "}
              Chờ lịch phát tiếp theo...
            </div>
          )}
        </footer>
