...
export const PeriodsTab = () => {
  ...
  const {
    ...
    volume,
    setVolume,
    globalFadeInDuration,
    setGlobalFadeInDuration,
    orgMode,
    setOrgMode,
    fileUploading,
    setFileUploading,
    ...
    changeOrgMode,
    fetchUsers,
    resumeYtVideoOnPlayer,
    pauseYtVideoOnPlayer,
    stopYtVideoOnPlayer,
    handleYtInputKeyDown,
    fastPlayYt,
  } = ctx;
  ...
  const saveBulk = async () => {
    if (!bulkDep || !bulkAudio)
      return notify(
        "Vui lòng chọn Khu vực và Âm thanh chuông ở Form tự động!",
        "err",
      );
    if (bulkPreview.length === 0)
      return notify(
        "Cụ phải bấm nút Xem trước màu xám trước thì hệ thống mới có dữ liệu để Lưu ạ!",
        "err",
      );
    try {
  ...
  const savePeriod = async () => {
    if (
      ...
      return notify(
        "Vui lòng điền đủ Tên, Khu vực, Thời gian và Âm thanh cho phần Thêm thủ công!",
        "err",
      );
    try {
      if (editingPeriod) {
        await api.put(`/api/periods/${editingPeriod.id}`, {
          ...pForm,
          departmentId: Number(pForm.departmentId),
          audioFileId: Number(pForm.audioFileId),
        });
        setEditingPeriod(null);
        notify("Đã cập nhật tiết!");
      } else {
        await api.post("/api/periods", {
          ...pForm,
          departmentId: Number(pForm.departmentId),
          audioFileId: Number(pForm.audioFileId),
        });
  ...
  const openEdit = (p: any) => {
    ...
    setPForm({
      ...
      startTime: p.startTime,
      endTime: p.endTime,
      audioFileId: String(p.audioFileId),
      volume: p.volume,
      isActive: p.isActive,
      daysOfWeek: p.daysOfWeek,
    });
  ...
  const handleBulkUpdatePeriods = async () => {
    ...
    if (bulkEditPeriodForm.audioFileId)
      payload.audioFileId = Number(bulkEditPeriodForm.audioFileId);
    if (bulkEditPeriodForm.departmentId)
      payload.departmentId = Number(bulkEditPeriodForm.departmentId);
    if (bulkEditPeriodForm.daysOfWeek)
      payload.daysOfWeek = bulkEditPeriodForm.daysOfWeek;
    ...
    try {
      ...
      fetchPeriods();
      notify(
        `Đã sửa hàng loạt thành công ${selectedPeriods.length} ${curProfile.itemUnit}!`,
      );
    } catch (err: any) {
      notify(
        err.response?.data?.error || "Lỗi sửa hàng loạt " + curProfile.itemUnit,
        "err",
      );
    }
  };
  ...
  const bulkDelete = async () => {
    if (selectedPeriods.length === 0) return;
    if (!(await customConfirm(`Xóa ${selectedPeriods.length} tiết đã chọn?`)))
      return;
    try {
  ...
  const bulkToggleActive = async (isActive: boolean) => {
      ...
      fetchPeriods();
      notify(
        `Đã ${isActive ? "bật" : "tắt"} ${selectedPeriods.length} ${curProfile.itemUnit}!`,
      );
    } catch {
  ...
  const toggleAll = () =>
    setSelectedPeriods(
      selectedPeriods.length === periods.length ? [] : periods.map((p) => p.id),
    );
      ...
      {showBulkEditPeriod && (
        <div className="modal-overlay" style={{ zIndex: 1000 }}>
          ...
          >
            ...
            >
              Thay đổi đồng loạt {selectedPeriods.length} {curProfile.itemUnit}{" "}
              đã chọn.
            </p>
...
/>
              ...
              <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                ...
                <DayPicker
                  value={pForm.daysOfWeek}
                  onChange={(v) => setPForm({ ...pForm, daysOfWeek: v })}
                />
              </div>
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setPForm({ ...pForm, isActive: e.target.checked })
                  }
                  style={{ width: "18px", height: "18px", cursor: "pointer" }}
                />
                <label
                  htmlFor="edit-p-active"
            ...
            >
              <button className="btn btn-primary" onClick={savePeriod}>
                {React.createElement("ion-icon", { name: "save-outline" })} Lưu
                thay đổi
              </button>
              <button
      ...
      <div
        className="card"
        style={{ marginBottom: "2rem", borderTop: "4px solid var(--accent)" }}
      >
        <h3
          style={{
            display: "flex",
        ...
        >
          ...
          >
            <div className="form-group">
              ...
              >
                {React.createElement("ion-icon", { name: "text-outline" })} Tên{" "}
                {curProfile.itemUnit}
              </label>
              <input
                type="text"
                ...
                placeholder={`${curProfile.itemBaseDefault} 1`}
              />
            </div>
            <div className="form-group">
              <label
              ...
              >
                {React.createElement("ion-icon", {
                  name: curProfile.departmentIcon,
                })}{" "}
...
/>
          ...
          >
            ...
            >
              <label
                style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}
              >
                {React.createElement("ion-icon", { name: "calendar-outline" })}{" "}
                Ngày trong tuần
              </label>
              <DayPicker
                value={pForm.daysOfWeek}
              ...
              >
                <button
                  ...
                  onClick={() =>
                    setPForm({ ...pForm, daysOfWeek: ALL_WEEKDAYS })
                  }
                >
                  T2–T6
                </button>
                <button
                  type="button"
                  className="btn btn-xs btn-outline"
                  onClick={() => setPForm({ ...pForm, daysOfWeek: ALL_DAYS })}
                >
                  Tất cả
                </button>
              </div>
      ...
      <div
        ...
        style={{
          ...
          background:
            "linear-gradient(to bottom right, var(--card-bg), rgba(139, 92, 246, 0.03))",
        }}
      >
        <h3
          style={{
...
/>
              ...
              <div className="form-group" style={{ margin: 0 }}>
                ...
                >
                  <button
                    type="button"
                    className={`btn btn-xs ${bulkDays === ALL_WEEKDAYS ? "btn-primary" : "btn-outline"}`}
                    onClick={() => setBulkDays(ALL_WEEKDAYS)}
                  >
                    T2–T6
                  </button>
                  <button
                    type="button"
                    className={`btn btn-xs ${bulkDays === ALL_DAYS ? "btn-primary" : "btn-outline"}`}
                    onClick={() => setBulkDays(ALL_DAYS)}
                  >
                    Tất cả
                  </button>
                </div>
              ...
              {bulkLongBreaks.map((lb, idx) => (
                ...
                >
                  <span style={{ fontSize: "0.85rem" }}>
                    Sau {curProfile.itemBaseDefault}
                  </span>
                  <input
                    type="number"
                    className="input"
                  ...
                  <button
                    ...
                    onClick={() => {
                      setBulkLongBreaks(
                        bulkLongBreaks.filter((_, i) => i !== idx),
                      );
                    }}
                    title="Xóa"
                  >
        ...
        >
          ...
          {bulkPreview.length > 0 && (
            <button className="btn btn-primary" onClick={saveBulk}>
              ...
              Lưu {bulkPreview.length} {curProfile.itemUnit}
            </button>
          )}
          {bulkPreview.length > 0 && (
            <button
        ...
        {bulkPreview.length > 0 && (
          ...
          >
            ...
            >
              Bảng xem trước ({bulkPreview.length} {curProfile.itemUnit})
            </h4>
            <table
              style={{
                width: "100%",
      ...
      <div className="card">
        ...
        >
          ...
          {selectedPeriods.length > 0 && (
            ...
            >
              ...
              >
                Đã chọn {selectedPeriods.length} {curProfile.itemUnit}
              </span>
        ...
        {periods.length === 0 && (
          <div className="empty-state">Chưa có {curProfile.itemUnit} nào</div>
        )}
        {periods.length > 0 && (
                      ...
                      >
                        <span
                          style={{
                            ...
                            height: "10px",
                            borderRadius: "50%",
                            background: p.department?.color || "var(--primary)",
