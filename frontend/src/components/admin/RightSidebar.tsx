...
export const RightSidebar = () => {
  ...
  const volWrapperRef = useRef<HTMLDivElement>(null);
  ...
  // Scroll wheel volume control: 2% per tick
  useEffect(() => {
    const el = volWrapperRef.current;
    if (!el) return;
    const onWheel = (e: WheelEvent) => {
      ...
      const newVal = Math.max(0, Math.min(1, volume + delta));
